...
class ConvTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        logger.info("ama embd {}".format(str(n_embd)))
    ...
    def forward(self, x, mask, use_xl=False):
        ...
        if isinstance(self.n_in, (list, tuple)):
            x = torch.cat(
                [proj(s, mask)[0] for proj, s in zip(self.proj, x.split(self.n_in, dim=1))], dim=1
            )
        ...
        for idx in range(len(self.branch)): #  1 512 [1024 512 256 128 64 32]
            if self.use_xl:
                if idx == 0: 
                    # =================== xlnet ==========================#
                    x = x.permute(0,2,1)
